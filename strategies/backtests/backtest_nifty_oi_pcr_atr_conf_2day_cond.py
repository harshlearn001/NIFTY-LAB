...
ATR Stop + Confidence Sizing + Conditional 2-Day Hold (Low Volatility)
----------------------------------------------------------------------
- Entry: Close on signal day
- Exit:
    * If Day-1 favorable AND ATR/Close < 1% → exit Day-2 close
...
OUT_FILE = PROC_DIR / "futures_ml" / "backtest_nifty_oi_pcr_atr_conf_2day_cond.parquet"
...
for i in range(len(df) - 2):  # need i+2
    row = df.iloc[i]
    ...
    position_size = (equity * row["risk_pct"]) / (stop_dist / entry)
    ...
    if ret_d1 < -max_loss:
        ...
        # Conditional 2-day hold: winner + low vol
        if (ret_d1 > 0) and (row["atr_pct"] < LOW_VOL_THRESHOLD):
            ret_d2 = (close_d2 - entry) / entry
            if row["final_signal"] == "SELL":
                ret_d2 *= -1
            ret = max(ret_d2, -max_loss)
        else:
            ret = ret_d1
...
# RESULTS
# =================================================
print("\n📊 BACKTEST RESULTS (CONDITIONAL 2-DAY HOLD)")
